...
(function spotifyAudioListenerTag() {
  ...
  window.addEventListener('message', function(event) {
    ...
    var spotifyEventString = event.data.payload
      ...
      + JSON.stringify(Math.trunc(event.data.payload.duration))
      + JSON.stringify(Math.trunc(event.data.payload.position))
      + JSON.stringify(event.data.payload.playingUri)) || '';
    // Leave if current event is the same as last event, otherwise just update
...
```json
{
    ...
    "exportTime": "2025-06-21 05:26:23",
    "containerVersion": {
