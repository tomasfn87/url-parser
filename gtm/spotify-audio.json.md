...
(function spotifyAudioListenerTag() {
  ...
  window.addEventListener('message', function(event) {
    if (event.origin === 'https://open.spotify.com') {
      ...
      if ((spotifyURI && spotifyURI !== spotifyLastURI) || (spotifyAudioCompleted && (!audioDuration || ['track', 'episode'].includes(audioData.contentType))) || (spotifyURI && spotifyURI === spotifyLastURI && Math.trunc(audioDuration) !== Math.trunc(spotifyLastDuration))) {
...
```json
{
    ...
    "exportTime": "2025-05-31 20:06:31",
    "containerVersion": {
